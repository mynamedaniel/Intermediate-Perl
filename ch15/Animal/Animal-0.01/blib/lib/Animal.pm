...
sub name {
	my $name = shift;
	ref $name ? $name->{Name} : "an unnamed $name";
}
...
sub speak {
	my $animal = shift;
	print $animal->name, ' goes ', $animal->sound, "\n";
}
...
sub color {
	ref $_[0] ? $_[0]->{Color} : $_[0]->default_color;
}
...
sub set_color {
	ref ($_[0]) or die "instance variable needed";
	$_[0]->{Color} = $_[1];
}
...
Daniel Z XU, C<< <mynamedaniel at 126.com> >>
